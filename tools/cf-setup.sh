#!/usr/bin/env bash
# One-time Cloudflare setup for CFB/26 accounts. Run from the repo root after `npx wrangler login`.
...
if ! grep -q 'database_id = "REPLACE_ME"' worker/wrangler.toml; then
  ...
  ID=$($W d1 list --json 2>/dev/null | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{try{const j=JSON.parse(s);const d=j.find(x=>x.name==="cfb26");console.log(d?d.uuid:"")}catch{console.log("")}})')
  if [ -z "$ID" ]; then
    OUT=$($W d1 create cfb26 2>&1) || { echo "$OUT"; exit 1; }
    ID=$(echo "$OUT" | grep -oE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' | head -1)
    echo "   created cfb26 ($ID)"
  else
    echo "   found existing cfb26 ($ID)"
  fi
...
echo "→ Tables…"
$W d1 execute cfb26 --remote --file=worker/schema.sql -c worker/wrangler.toml -y >/dev/null
echo "   ok"
