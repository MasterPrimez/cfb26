...
const SIGMA = 13.86; // std dev of CFB margin vs spread
...
export async function loadHome(ctx, teamId) {
  ...
  const apRank = ctx.rankings?.ap?.ranks.find(r => String(r.team.id) === String(teamId));
  const cfpIdx = ctx.playoff?.field.findIndex(t => t.id === String(teamId)) ?? -1;
  const confTeams = conf ? ctx.directory.teams.filter(t => t.conf.id === conf.id).sort((a, b) => b.wins - a.wins || (b.pointsFor - b.pointsAgainst) - (a.pointsFor - a.pointsAgainst)) : [];
  ...
  const out = { t: Date.now(), team, opp, next, game, past, events, prob, photos: { mine: myVenue, opp: oppVenue }, apRank: apRank?.current || null, cfpSeed: cfpIdx >= 0 ? cfpIdx + 1 : null, cfpOfficial: !!ctx.playoff?.official, confPlace, confTeams, rankHist, remaining: events.filter(e => e.state !== 'post').length };
  ctx.home[teamId] = out;
  return out;
}
...
export function renderHome(ctx, d, opts = {}) {
  ...
  const { team, opp, next, game, past, prob, apRank, cfpSeed, cfpOfficial, confPlace, rankHist, remaining } = d;
  ...
  const standSec = `<section class="story-sec" id="standing">
      ...
      <div class="tiles">
        <div class="rv d1"><div class="disp v">${apRank ? '#' + apRank : 'NR'}</div><div class="k">AP Poll</div></div>
        <div class="rv d2"><div class="disp v">${cfpSeed ? cfpSeed : '—'}</div><div class="k">CFP seed${cfpOfficial ? '' : ' · proj'}</div></div>
        <div class="rv d3"><div class="disp v">${confPlace ? ord(confPlace) : '—'}</div><div class="k">${esc(team.conf?.abbr || 'Conference')}</div></div>
  ...
  const deeperSec = `<section class="story-sec" id="deeper" style="min-height:80vh">
      ...
      <div class="grid4">
        <a class="tile rv d1" href="#/scores"><div class="disp t">All scores</div><div class="k">EVERY FBS GAME · LIVE</div></a>
        <a class="tile rv d2" href="#/tv"><div class="disp t">TV guide</div><div class="k">WHAT'S ON, BY NETWORK</div></a>
        <a class="tile rv d3" href="#/playoff"><div class="disp t">Rankings &amp; playoff</div><div class="k">AP · COACHES · BRACKET</div></a>
        <a class="tile rv d4" href="#/team/${team.id}"><div class="disp t">${esc(team.name)}</div><div class="k">SCHEDULE · STATS · STANDINGS</div></a>
      </div>
      <div class="mono foot">CFB/26 · DATA: ESPN · ${prob?.src === 'vegas' ? 'WIN PROBABILITY FROM THE BETTING LINE' : prob?.src === 'model' ? 'WIN PROBABILITY IS OUR MODEL' : ''}</div>
    </section>`;
