...
export const TILE_CATALOG = [
  ...
  { id: 'close',     name: 'Closest games',         blurb: 'One-score games in the 4th quarter.' },
...
export function renderDash(ctx, d) {
  ...
  const closest = liveGames.map(g => ({ g, m: Math.abs((g.home.score || 0) - (g.away.score || 0)) })).filter(x => x.m <= 8 && ((x.g.period && x.g.period >= 4) || /4th|OT/i.test(x.g.detail || ''))).sort((a, b) => a.m - b.m).slice(0, 4);
  const closeTile = `<div class="dt close pad" data-href="#/scores"><div class="k">CLOSEST GAMES<span class="svc"> · 4TH QUARTER, ONE SCORE</span><span class="more">SCORES</span></div>${closest.map(({ g, m }) => `<a class="ug" href="#/game/${g.id}"><span class="tm"><img src="${esc(g.away.logo)}" alt=""><em>${esc(g.away.name)}</em><b>${g.away.score}</b></span><span class="tm"><img src="${esc(g.home.logo)}" alt=""><em>${esc(g.home.name)}</em><b>${g.home.score}</b></span><span class="li">● ${esc(g.detail)}${m === 0 ? ' · TIED' : ''}</span></a>`).join('') || `<div class="muted mono" style="font-size:11px">${liveGames.length ? 'No nail-biters yet.' : 'Nothing live right now.'}</div>`}</div>`;
  ...
  const livegridTile = !lgGames.length && !document.body.classList.contains('dash-edit') ? '' : `<div class="dt livegrid pad" style="grid-row:span ${isMobile() ? 1 : Math.ceil((60 + lgRows * 172) / 108)}"><div class="k">LIVE SCOREBOARD · ${lgGames.length} GAME${lgGames.length === 1 ? '' : 'S'} ON<span class="svc"> · RED GLOW = ONE-SCORE GAME IN THE 4TH</span><a class="more" href="#/scores">ALL SCORES</a></div><div class="lg2">${lgGames.map(lgCard).join('') || '<div class="muted mono" style="font-size:11px">Nothing live right now — this tile fills up at kickoff.</div>'}</div></div>`;
