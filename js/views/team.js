...
export async function renderTeam(ctx, params) {
  ...
  const confTeams = conf ? d.teams.filter(t => t.conf.id === conf.id).sort((a, b) => b.wins - a.wins || (b.pointsFor - b.pointsAgainst) - (a.pointsFor - a.pointsAgainst)) : [];
    ...
    <div class="grid-main">
      ...
      <aside class="rail">
        ${conf ? `<div class="panel panel-pad"><div class="disp h3" style="margin-bottom:10px">${esc(conf.name)} Standings</div>
          <table><thead><tr><th>Team</th><th style="text-align:right">Conf</th><th style="text-align:right">Ovr</th><th style="text-align:right">Strk</th></tr></thead><tbody>
          ${confTeams.map(t => `<tr${t.id === String(id) ? ' style="background:rgba(245,165,36,0.06)"' : ''}><td><a href="#/team/${t.id}"${t.id === String(id) ? ' class="amber"' : ''}>${esc(t.name)}</a></td><td class="mono" style="text-align:right">${esc(t.confRec || '0-0')}</td><td class="mono" style="text-align:right">${esc(t.overall)}</td><td class="mono muted" style="text-align:right">${esc(t.streak)}</td></tr>`).join('')}
          </tbody></table></div>` : ''}
      </aside>
    </div>`;
}
